-- The two real case studies, moved from the repository into the database.
--
-- Until this runs, the homepage and the CMS read from different places: the
-- homepage falls back to the copies committed in app/lib/caseStudies.ts when
-- the table is empty, while the editor only ever reads the table. That is why
-- the site shows two case studies and the CMS shows none.
--
-- The logo and video paths are left pointing at /public. publicUrl() passes
-- any path beginning with "/" straight through, so both the homepage and the
-- editor render them exactly as before and nothing has to be uploaded to
-- storage first. Replacing either through the editor uploads it properly.
--
-- Wrapped in a guard rather than ON CONFLICT, because the position constraint
-- is deferrable and so cannot arbitrate a conflict. Re-running is a no-op.
