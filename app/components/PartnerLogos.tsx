...
function LogoNode({ name }: { name: string }) {
  if (name === "Northbridge Construction") {
    return (
      <div className="flex items-center gap-1">
        ...
        <div className="text-[9px] font-semibold leading-tight tracking-wide">
          NORTHBRIDGE<br />
  ...
  if (name === "Atlas Civil") {
    return (
      <div className="flex items-center gap-1">
        ...
        <div className="text-[9px] font-semibold leading-tight tracking-wider">
          STATE OF ATLAS<br />
  ...
  if (name === "DARS") {
    return (
      <div className="flex flex-col items-center">
        <svg viewBox="0 0 60 24" className="h-3 w-7">
          <path d="M2 22 Q 30 -4 58 22" stroke="currentColor" strokeWidth="2" fill="none" />
        </svg>
        <div className="mt-0.5 text-[12px] font-bold tracking-tight">DARS</div>
  ...
  if (name === "The Great Seal") {
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="mt-0.5 text-[8px] font-medium tracking-widest opacity-80">THE GREAT SEAL</div>
      </div>
    );
  }
  if (name === "Ironwood") {
    return <div className="text-[15px] font-serif italic tracking-tight">Ironwood</div>;
  }
  if (name === "MetroDOT") {
    return (
      <div className="flex flex-col items-center">
        <div className="text-[11px] font-bold tracking-tight">MetroDOT</div>
  ...
  if (name === "dss Public Works") {
    return (
      <div className="flex flex-col items-start">
        <div className="text-[15px] font-bold lowercase italic">dss</div>
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-0.5 text-[10px] font-bold tracking-tight">SUMMIT</div>
    ...
    <section className="sticky bottom-0 z-10" style={{ background: "#0b0a0c" }}>
      ...
      <div className="hidden lg:block w-full px-8 py-3 md:px-16">
        <div className="mx-auto max-w-[var(--container-content)] grid grid-cols-10 items-center gap-4">
          {logoNames.map((name) => (
            <div key={name} className="flex h-10 items-center justify-center text-white/80" aria-label={name}>
              <LogoNode name={name} />
            </div>
          ))}
        </div>
      ...
      <div className="lg:hidden relative py-3" style={{ overflow: "hidden" }}>
        ...
        <div className="logo-ticker-track flex items-center" style={{ width: "max-content" }}>
          {[...logoNames, ...logoNames].map((name, i) => (
            <div
              key={i}
              className="flex h-10 w-40 flex-shrink-0 items-center justify-center px-3 text-white/80"
